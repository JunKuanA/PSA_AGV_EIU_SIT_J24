...
public class ProcessMsg {
    private static Logger logger = Logger.getLogger(ProcessMsg.class.getName());
    ...
    public ProcessMsg(ESBClient esbClient, persistenceManager dbCtrl,HashMap<String, Object> hmDbParam,OpcUaClient opcUaClient, Wago wago) {
    ...
    public ProcessMsg(ESBClient esbClient, persistenceManager dbCtrl,HashMap<String, Object> hmDbParam) {
        this.esbClient = esbClient;
        this.dbCtrl = dbCtrl;
        this.hmDbParam = hmDbParam;
        if (hmDbParam.size()>0){hmDbParam.clear();}
    ...
    public void onMessageReceived(String esbTransId, String payloadX) {
        payload = null;
        gson = new Gson();
        // wago = new Wago();
        ...
        try {
            ...
            logger.error(String.format("EIUProcessStep1 - onMessageReceived - payloadX: %s", payloadX), e);
            return;
        }
        ...
        if (esbTransId.equalsIgnoreCase(ConfigWago.getEsbJcEnqInfo())){
            ...
            try{
                ...
                if (wago.getWagoStatusCode() == 0) {
                    ...
                    int heartbeatRunningNumber = equipmentInfo.get(0).getLast_hb();
                    try {
    ...
    private void verifyTopic(String esbTransId) {
        ...
        if (esbTransId.equalsIgnoreCase(ConfigWago.getEsbJcUpdSt())){
            ...
            mnl_xing_req_i = jcEnqInfoMsg.getMnl_xing_req_i();
            jcEnqInfoMsg.setMnl_xing_req_i(mnl_xing_req_i);
